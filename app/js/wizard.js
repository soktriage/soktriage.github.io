...
(function () {
  ...
  function lejartAdatTisztitas() {
    ...
    biztos(function () {
      ...
      var ts = raw.charAt(0) === '{' ? (JSON.parse(raw) || {}).ts : null;
      if (lejart(ts)) { localStorage.removeItem(STORE_STAFF_AKTIV); torolt.push('aktív ápoló neve'); db++; }
    });
  ...
  function torlodasSavRajzol() {
    ...
    var ki = el('button', 'torl-ki', 'Kikapcsolás'); ki.type = 'button';
  ...
  RENDER.betegut = function (fo) {
    ...
    if (szint === 1 || szint === 2) {
      ...
      if (torlAktiv) {
        ...
        (TO.utak || []).forEach(function (u) {
          ...
          d.appendChild(el('summary', null, u.sorszam + '. ' + u.cim));
    ...
    if ((KB.tek || {}).telepulesek) {
      ...
      var tekBtn = elIko('button', 'btn btn-ghost btn-full', 'search', 'Területi ellátás (TEK) — kerület / település');
        ...
        cs.forEach(function (v) {
          ...
          if (ra) {
            ...
            r.textContent = ra.lejart
              ? ('RE-TRIAGE ESEDÉKES — ' + ra.eltelt + ' perce (' + ra.intervallum + ' percenként)')
              : ('következő re-triage ' + ra.hatra + ' perc múlva (' + ra.intervallum + ' percenként)');
            bal.appendChild(r);
          } else if (v.szint === 1) {
  ...
  function varolistaSzoveg() {
    var L = ['MENTŐÁGYON VÁRÓ BETEGEK — re-triage lista (' + oraPerc(Date.now()) + ')'];
    var t = torlodas();
    ...
    varolista().slice().sort(function (a, b) { return varolistaSzint(a) - varolistaSzint(b) || a.erkezes - b.erkezes; })
      .forEach(function (v) {
        var ra = retriageAllapot(v);
        L.push('- MSTR ' + (v.szint == null ? '?' : v.szint) + ' · ' + v.cimke + (v.panasz ? ' (' + v.panasz + ')' : '') +
  ...
  function tudastarSzekciok() {
    ...
    if ((K.tek || {}).telepulesek) ki.push({ id: 'tek', cim: 'Területi ellátás (TEK)', leiras: 'Kerület / település → mely SE-klinikák látják el. Offline kereső.', db: (K.tek.keruletek || []).length + (K.tek.telepulesek || []).length, overlay: true });
    ...
    if ((K.betegut || {}).felvetelSzabalyok) ki.push({ id: 'felvetel', cim: 'SE SOK betegfelvételi alapelvek', leiras: 'A klinika 10 felvételi alapelve.', db: ((K.betegut.felvetelSzabalyok || {}).pontok || []).length });
  ...
  function tudastarTetelek() {
    ...
    (K.vitalBands || []).forEach(function (b) {
      t.push({ szekcio: 'gyermek', cim: b.name, szoveg: b.kiertekeles, savok: b.rows, forras: b.source });
    });
  ...
  RENDER.tudastar = function (fo) {
    ...
    function rajzol() {
      ...
      if (q) {
        ...
        if (tal.length > 40) { var tb = el('div', 'skip-hint'); tb.textContent = '… és további ' + (tal.length - 40) + ' találat — pontosítsd a keresést.'; tart.appendChild(tb); }
        return;
      }
      if (!S.tudSzekcio) {
        szek.forEach(function (s) {
  ...
  RENDER.tetra = function (fo) {
    S.tetra = S.tetra || {};
    var T = S.tetra;
    var c = kartya('TETRA', 'Adatfelvételi lap — TETRA / telefon riasztás', 'Prehospitális riasztás rögzítése (SBAR) — a hivatalos nyomtatvány szerint.');
    ...
    gv.appendChild(tInput('gcs', 'GCS (Sz-V-M)', 'pl. 4-5-6', true));
    ...
    if (T.jStroke) {
      ...
      if (rp != null) {
        ...
        sc.appendChild(lvo);
        if (rp >= 5) sc.appendChild(tToggle('stINKertesitve', 'INK értesítve'));
      }
